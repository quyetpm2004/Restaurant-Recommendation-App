...
import { dishOptionGroups } from '../data/mockData';
...
export function DishSelector({ selected, onChange }: DishSelectorProps) {
  ...
  const visibleCount = filteredGroups.reduce((sum, g) => sum + g.dishes.length, 0);
  const totalCount = dishOptionGroups.reduce((sum, g) => sum + g.dishes.length, 0);

  const toggleDish = (dish: string) => {
    onChange(
      selected.includes(dish) ? selected.filter((d) => d !== dish) : [...selected, dish]
    );
  };
                    ...
                    {group.dishes.map((dish) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className="text-sm text-gray-800 leading-tight">{dish}</span>
                        </div>
                      );
                    })}
                  </div>
